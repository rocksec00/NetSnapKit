...
# Auto-increment filename to prevent overwrite
...
# Add RockSec banner and URL label
def add_label_to_image(image_bytes, label):
    ...
    label_height = 80  # Space for banner + domain
    banner_height = 30

    # New image with space for label
    new_image = Image.new("RGB", (image.width, image.height + label_height), (255, 255, 255))
    new_image.paste(image, (0, label_height))
    draw = ImageDraw.Draw(new_image)

    # Banner background
    draw.rectangle([(0, 0), (image.width, banner_height)], fill=(0, 0, 64))  # Dark blue

    # Domain label background
    draw.rectangle([(0, banner_height), (image.width, label_height)], fill=(0, 0, 0))  # Black
...
async def capture_urls(urls, output_pdf_name):
    ...
    if screenshot_images:
        ...
        print(f"[✔] Screenshots saved to PDF: {pdf_path}")
    else:
